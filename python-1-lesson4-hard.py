...
def withdraw_money(person, money):
    if person['money'] - money >= 0:  # остаток должен быть больше либо равно 0, поэтому знак меняется на >=
        person['money'] -= money
...
def process_user_choice(choice, person):
    if choice == 1:  # передается int-значение, поэтому кавычки не нужны
        print(check_account(person))
    elif choice == 2:  # передается int-значение, поэтому кавычки не нужны
        count = float(input('Сумма к снятию:'))
        print(withdraw_money(person, count))


def start():
    try:  # если данные введены с ошибками изначально, то лучше сразу включить ввод данных в конструкцию try-except
        card_number, pin_code = input('Введите номер карты и пин код через пробел:').split(
            ' ')  # разделительным знаком является пробел, поэтому указываем его.
    ...
    except ValueError:  # если неправильно ввести данные, то выходит ошибка ValueError. Ошибок другого типа не обнаружил
